function granite_table(tableBlock, jsonTheme) {
  ...
  const t = jsonTheme; // Color values
...
---------------------------------------------*/
  ...
  (function (root, factory) {
    ...
    function cellFormat(record) {
      ...
      switch (record['type']) {
        ...
        case 'date':
          ...
          var editable = record['editable'] === true ? `<i class="far fa-calendar-alt g__calendar_btn"></i>` : ""; // cell = `${hidden}<input class="g__inline_input" id='${id}' type="text" value="${date}">${editable}`;
    ...
    var DataTable = function (table, options) {
      ...
      if (table.tHead === null) {
        if (!this.options.data || this.options.data && !this.options.data.records) {
          this.options.sortable = false;
        }
      }
    ...
    proto.render = function (type) {
      ...
      var g_header = createElement('div', {
        class: "g__datatable_header"
      });
      var dt_header = '';
      ...
      template = template.replace("{search}", form); // BUTTONS

      var button_container = createElement("div", {
        class: "g__button_container mr-2"
      });
      ...
      template = template.replace("{button_container}", button_container.outerHTML); // Exportable
      ...
      o.padded ? classList.add(that.table, "table-padded") : classList.add(that.table, "table-normal"); // Paginator
    ...
    proto.fixHeight = function () {
      if (this.options.fixedHeight) {
        this.container.style.height = null; // this.rect = this.container.getBoundingClientRect();
  ...
  var dt_search = document.querySelectorAll('.search-container .dataTable-input'); // console.log(dt_search.length)
